...
use yellowstone_vixen_block_coordinator::CoordinatorHandle;

use crate::{
    events::PreparedRecord,
    sink::{ConfiguredParsers, ParsedInstruction},
    utils::get_all_ix_with_index,
};
        ...
        for filter in self.parsers.secondary_filters() {
            if let Some(filtered) = filter.filter(ix, primary_parsed).await {
                let record = self.parsers.prepare_decoded_record(
                    slot, &ix.shared.signature, path, filtered, filter.label(), filter.topic(),
                );
                if let Err(e) = self
                    .handle
                    .send_parsed(slot, tx_index, path.to_vec(), record)
                    .await
                {
        ...
        for (ix_index, ix_update) in instructions.iter().enumerate() {
            for (path, ix) in get_all_ix_with_index(ix_update, ix_index) {
                let (record, primary_parsed) = self
                    .parsers
                    .parse_instruction(slot, &ix.shared.signature, &path, ix)
                    .await;

                if let Err(e) = self
                    .handle
                    .send_parsed(slot, tx_index, path.clone(), record)
                    .await
                {
                    tracing::error!(?e, slot, tx_index, "Failed to send parsed record");
                }

                self.apply_secondary_filters(slot, tx_index, &path, ix, primary_parsed.as_ref())
                    .await;
            }
        }
